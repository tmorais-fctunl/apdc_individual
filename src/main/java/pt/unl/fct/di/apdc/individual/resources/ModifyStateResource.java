...
public class ModifyStateResource {
	...
	public Response doModifyState(ModifyStateData data) {
		...
		try {
			...
			if (userToChangeKey==null) {
				txn.rollback();
				return Response.status(Status.BAD_REQUEST).entity("User to change state does not exist").build();
			}

			if(!(data.getState().equals("DISABLED") | data.getState().equals("ENABLED"))) {
				txn.rollback();
				return Response.status(Status.BAD_REQUEST).entity("Invalid state").build();
			}

			if (user.getString("user_role").equals("SU") && !userToChange.getString("user_role").equals("SU")) {
				//Can update role for anyone except SU
				UpdateUserContent(txn, userToChangeKey, userToChange, data.getState());
				return Response.status(Status.ACCEPTED).entity("Success! User "+userToChange.getString("user_name")+"'s state changed to "+data.getState()+".").build();
			}
			...
			if (user.getString("user_role").equals("GBO") & userToChange.getString("user_role").equals("USER")) {
				//can update
				UpdateUserContent(txn, userToChangeKey, userToChange, data.getState());
				return Response.status(Status.ACCEPTED).entity("Success! User "+userToChange.getString("user_name")+"'s state changed to "+data.getState()+".").build();
			}
	...
	private void UpdateUserContent(Transaction transaction, Key userKey, Entity user, String state) {
